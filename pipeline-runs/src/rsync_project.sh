...
##PATH2_JSON_FILES=/data/projects/Biocore/biocore-pipelines/rna-seq/Mouse_KO_2019

## Where we will store the pipeline meta config file for each sample
## filename format:sampleID.organism.pcf
##PIPELINE_META_BASE=/data/projects/Biocore/biocore-pipelines/pipeline-runs-meta
...
## ${rsync_script}  $reads_base $AWS_SCRATCH_READS_BASE/$PROJECT_TEAM_NAME/
...
##${rsync_script} $dataset_base/$ORGANISM* $dataset_base/
  ...
  do
    ...
    then
       ...
       echo `date`" - ERROR $database_version reference indexes for $indexer not found" | tee -a $log
        ...
        ##${rsync_script} $local_index_base/$ORGANISM* $aws_index_base/
    fi
    echo "" | tee -a $log
  done
fi
